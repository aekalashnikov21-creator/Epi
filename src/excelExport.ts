...
export async function downloadExcel(): Promise<void> {
  ...
  r = block(
    ...
    ["Параметр", "Значение"],
    [
      ["Бюджет", "1 100 000 ₽/мес · 14 каналов"],
      ...
      ["ROMI 1-го месяца", "+241%"],
      ["LTV-маржа когорты · 12 мес", "17 152 000 ₽ · LTV-ROMI +1 459%"],
      ["Средний CPL / CAC", "410 ₽ / 1 026 ₽ · безубыточный CAC 16 000 ₽ (запас ×15.6)"],
    ],
    [34, 96],
    { zebra: true }
  );
  ...
  r = block(
    ...
    { totalLast: true, zebra: true, alignRight: [1, 2, 3, 4, 5, 6, 7, 8, 9], note: "Google Maps (органика) работает без бюджета — 38 лидов в месяц даёт заполненный профиль с отзывами." }
  );
  ...
  r = block(
    ...
    scenarios.map((s) =>
      s.id === "be"
        ? ["Безубыточность", "1 100 000", "314 (CR 11.7%)", "3 500 ₽", "1 099 000 ₽", "≈ 0% · план окупается при CR 11.7%, плановые 40% дают запас ×3.4"]
