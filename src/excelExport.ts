...
export async function downloadExcel(): Promise<void> {
  ...
  let r = title(
    ...
    "Сеть клиник лазерной эпиляции и косметологии · Москва, 6 филиалов · план: 89 лидов в день на сеть, конверсия в приход 40%"
  );
  r = block(
    ws, r, "СТРАТЕГИЯ В ЦИФРАХ",
    ["Параметр", "Значение"],
    [
      ["Бюджет привлечения (полный)", "2 020 000 ₽/мес (медиа 1 100 000 + команда, сервисы, контент 920 000)"],
      ["Лидов в день · сеть", "89 (по 15 на филиал)"],
      ["Продаж в день · сеть", "36 (по 6 на филиал, конверсия в приход 40%)"],
      ["Лидов / продаж в месяц", "2 681 / 1 072"],
      ["Выручка 1-го месяца", "3 752 000 ₽"],
      ["ROMI 1-го месяца (полный бюджет)", "+86% (на медиабюджете +241%)"],
      ["LTV-маржа когорты · 12 мес", "17 152 000 ₽ · LTV-ROMI на полном бюджете +749%"],
      ["CAC · 3 уровня", "media 1 026 ₽ (запас ×15.6) · полный 1 880 ₽ (×8.5) · all-in ~2 400 ₽ (×6.7) · норма рынка ×3"],
      ["Безубыточный CAC", "16 000 ₽ = LTV-маржа"],
    ],
    [34, 96],
    { zebra: true }
  );
  ...
  r = block(
    ws, r, "СТРУКТУРА ЗАТРАТ НА ПРИВЛЕЧЕНИЕ · ₽/МЕС",
    ["Статья", "Сумма, ₽", "Комментарий"],
    [
      ...costStructure.map((c) => [c.item, c.sum, c.note]),
      ["ИТОГО затрат на привлечение", 2020000, "полный CAC = 2 020 000 ÷ 1 072 ≈ 1 880 ₽"],
    ],
    [36, 16, 78],
    {
      zebra: true,
      alignRight: [1],
      totalLast: true,
      note: "Аренда (~1,5 млн ₽/мес) в CAC не входит — это производственная константа: клиника существует и без привлечения. Она учитывается в точке безубыточности и марже; в «all-in»-взгляд добавляется отдельно (+540 ₽ на клиента).",
    }
  );
  r = block(
    ws, r, "ТРИ УРОВНЯ CAC И ЗАПАС ПРОЧНОСТИ",
    ["Уровень", "CAC, ₽", "Запас до безубыточности (16 000 ₽)", "Формула"],
    cacTiers.map((t) => [t.name, `~${fmt(t.value)}`, `${t.mult} · норма рынка ×3`, t.desc]),
    [26, 12, 34, 58],
    { zebra: true, alignRight: [1] }
  );
  r = block(
    ws, r, "ТОЧКА БЕЗУБЫТОЧНОСТИ СЕТИ",
    ["Показатель", "Значение", "Комментарий"],
    [
      ["Постоянные расходы сети", "~1 800 000 ₽/мес", breakEven.fixedNote],
      ["Маржа с процедуры", "2 100 ₽", "60% от чека 3 500 ₽"],
      ["Безубыточность", `${fmt(breakEven.procedures)} процедур/мес`, `${breakEven.perDayNetwork}/день на сеть · ${breakEven.perDayBranch} на филиал`],
      ["Поток от новых клиентов", `${fmt(breakEven.newFlow)} процедур/мес`, breakEven.newFlowNote],
    ],
    [34, 24, 72],
    {
      zebra: true,
      alignRight: [1],
      note: "Поток новых клиентов почти вдвое выше порога — с повторными визитами сеть безубыточна даже при полностью отключённой рекламе.",
    }
  );
  ...
  r = title(
    ...
    "1 100 000 ₽/мес → 2 681 лид → 1 072 продажи (конверсия в приход 40%) · ROMI = (выручка − бюджет) / бюджет"
  );
  r = block(
    ws, r, null,
    ...
    { totalLast: true, zebra: true, alignRight: [1, 2, 3, 4, 5, 6, 7, 8, 9], note: "ROMI каналов — на медиабюджет. ROMI 1-го месяца на полном бюджете привлечения (2 020 000 ₽) — +86%, по LTV-марже когорты — +749%. Google Maps (органика) работает без бюджета — 38 лидов в месяц даёт заполненный профиль с отзывами." }
  );
  ...
  r = block(
    ...
    [
      ["Лидов в день", "15", "447 в месяц"],
      ["Первичных клиентов", "179", "6/день · конверсия 40%"],
      ["Выручка 1-го месяца", "625 300 ₽", "179 × 3 500"],
      ["LTV-маржа · 12 мес", "2 858 700 ₽", "179 × 16 000"],
    ],
    [34, 20, 46],
    {
      zebra: true,
      note: "Бюджет на филиал — ~183 000 ₽/мес (1 100 000 ₽ на сеть из 6 филиалов). Умножьте на 6 — получите 89 лидов и 36 продаж в день на всю сеть.",
    }
  );
  ...
  r = block(
    ...
    scenarios.map((s) =>
      s.id === "be"
        ? ["Безубыточность", "2 020 000", "577 (CR 21.5%)", "3 500 ₽", "2 019 500 ₽", "≈ 0% · полный CAC 1 880 ₽ (запас ×8.5), all-in ~2 400 ₽ (×6.7) · норма рынка ×3 · плановые 40% дают запас ×1.9"]
